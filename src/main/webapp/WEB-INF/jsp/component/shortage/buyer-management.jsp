...
<style>
    ...
    .btnUploadFileBuyer:hover,
    .btnDownloadFileBuyer:hover {
        color: #f8f8f8;
    }
    ...
    #tblListBuyer td:first-child {
        width: 5%;
    }

    #tblListBuyer td:last-child {
        width: 5%;
    }
    ...
    #modal-delete-buyer .modal-body {
        font-size: 16px;
    }

    #modal-delete-buyer .modal-footer {
        padding-bottom: 10px;
    }
    ...
    #modal-update-buyer .form-group {
        margin-bottom: 10px;
    }

    #modal-update-buyer .form-group label {
        font-weight: bold;
    }

    #modal-update-buyer .modal-footer {
        padding-bottom: 10px;
    }
    ...
    #modal-upload-buyer .modal-footer {
    ...
    #tblListBuyer_wrapper .dt-buttons {
        display: none;
    }
</style>
...
<div class="row main-container">
    <div class="col-md-12">
        <h5 class="title-header">BUYER MANAGEMENT</h5>
    </div>
    <div class="col-md-12 table-content">
        <div class="redirect" style="display: flex; align-items: center; justify-content: flex-end;">
            <button class="btn pull-right btnDownloadFileBuyer"><i class="fa fa-download"></i> Export file</button>
            <button class="btn pull-right btnUploadFileBuyer" onclick="showModalUploadBuyer()"><i class="fa fa-upload"></i> Import file</button>
            <input type="text" id="inputSearch" placeholder="Search...">
        </div>
        <table class="table table-sm table-bordered" id="tblListBuyer">
            <thead>
                <th>#</th>
                <th>Factory</th>
                <th>BU</th>
...
<div class="modal fade" id="modal-upload-buyer">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <div class="" style="display: flex;align-items: center;justify-content: space-between;">
                    <a href="/assets/files/buyer_management.xlsx" class="linkDownloadBuyerFormat"><i class="fa fa-download"></i> Format file</a>
                    <button class="btn btn-sm btn-outline-secondary btnConfirmUploadUser" onclick="uploadBuyer()">Upload</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="modal-delete-buyer">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Delete Buyer</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body" style="background-color: #F7F7FF;">
                <div class="col-md-12">
                    <span>Are you sure want to delete buyer </span>
                    <span class="title-idcard-delete"></span>
                    <span>?</span>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-sm btn-outline-secondary btnConfirmDeleteUser" onclick="deleteBuyer()" data-dismiss="modal">Delete</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="modal-update-buyer">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
...
<script>
    ...
    function loadListBuyers() {
        if (tblListBuyer) {
            tblListBuyer.destroy();
        }
            ...
            success: function (res) {
                ...
                for (var i = 0; i < data.length; i++) {
                    html +=
                        ...
                        '<td>' + checkNull(data[i].buyer) + '</td>' +
                        '<td>' + checkNull(data[i].buyerCode) + '</td>' +
                        '<td>' + checkNull(data[i].buyerExt) + '</td>' +
                        '<td>' + checkNull(data[i].buyerEmail) + '</td>' +
                        '<td class="config">' + '<i class="fa fa-trash text-danger btnDeleteBuyer" onclick="showModalDeleteBuyer(\'' + data[i].buyerCode + '\',' + data[i].id + ')"></i>' + '</td>' +
                        '</tr>';
                }
                $('#tblListBuyer tbody').html(html);

                var page = 13;
                if (widthScreen >= 1920) {
                    page = 20;
                }
                tblListBuyer = $('#tblListBuyer').DataTable({
                    "paging": true,
                    "pageLength": page,
                    "lengthChange": false,
                    "searching": true,
                ...
                $('#inputSearch').on('keyup', function () {
                    tblListBuyer.search($(this).val()).draw();
                });
            },
            error: function (err) {
                console.log(err)
    ...
    function showModalDeleteBuyer(buyerCode, buyerId) {
        dataset.buyerId = buyerId * 1; //moi khi nguoi dung click vao nut xoa se dong thoi gan gia tri idUser
        $('.title-idcard-delete').html(buyerCode);
        $('#modal-delete-buyer').modal('show');
    }
    ...
    function showModalUpdateUser(context) {
        $('#modal-update-buyer').modal('show');
    ...
    function showModalUploadBuyer() {
        $('#modal-upload-buyer').modal('show');
    }

    function uploadBuyer() {
    ...
    function checkSMMSRole() {
        if (dataset.smmsRole == 'MPM') {
            $('.btnUploadFileBuyer').remove();
        } else if (dataset.smmsRole == 'BUYER') {
            $('.btnUploadFileBuyer').remove();
        }
    }

    $('.btnDownloadFileBuyer').on('click', function () {
        $('#tblListBuyer_wrapper .dt-buttons > .dt-button').trigger('click');
    });
