...
<style>
    ...
    .btnUploadFileUser:hover,
    .btnDownloadFileUser:hover {
        color: #f8f8f8;
    }
    ...
    #tblListUsers td:first-child {
        width: 5%;
    }

    #tblListUsers td:last-child {
        width: 5%;
    }
    ...
    #modal-delete-user .modal-footer {
        padding-bottom: 10px;
    }
    ...
    #modal-update-user .form-group {
        margin-bottom: 10px;
    }

    #modal-update-user .form-group label {
        font-weight: bold;
    }

    #modal-update-user .modal-footer {
        padding-bottom: 10px;
    }
    ...
    #modal-upload-user .modal-footer {
    ...
    #tblListUsers_wrapper .dt-buttons {
        display: none;
    }
</style>
...
<div class="row main-container">
    <div class="col-md-12">
        <h5 class="title-header">Users Management</h5>
    </div>
    <div class="col-md-12 table-content">
        <div class="redirect" style="display: flex; align-items: center; justify-content: flex-end;">
            <button class="btn pull-right btnDownloadFileUser"><i class="fa fa-download"></i> Export file</button>
            <button class="btn pull-right btnUploadFileUser" onclick="showModalUploadUser()"><i class="fa fa-upload"></i> Import file</button>
            <input type="text" id="inputSearch" placeholder="Search...">
        </div>
        <table class="table table-sm table-bordered" id="tblListUsers">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Factory</th>
...
<div class="modal fade" id="modal-upload-user">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <div class="" style="display: flex;align-items: center;justify-content: space-between;">
                    <a href="/assets/files/pm_user.xlsx" class="linkDownloadShippingModeFormat"><i class="fa fa-download"></i> Format file</a>
                    <button class="btn btn-sm btn-outline-secondary btnConfirmUploadUser" onclick="uploadUser()">Upload</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="modal-delete-user">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-sm btn-outline-secondary btnConfirmDeleteUser" onclick="deleteUser()" data-dismiss="modal">Delete</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="modal-update-user">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-sm btnConfirmUpdateUser" onclick="updateUser()" data-dismiss="modal">Update</button>
            </div>
        </div>
    </div>
</div>
...
<script>
            ...
            success: function (res) {
                ...
                for (var i = 0; i < data.length; i++) {
                    html +=
                        ...
                        '<td>' + checkNull(data[i].user) + '</td>' +
                        '<td>' + checkNull(data[i].role) + '</td>' +
                        '<td>' + checkNull(data[i].email) + '</td>' +
                        '<td class="config">' +
                        // '<i class="fa fa-edit btnUpdateUser" data-idcard="' + data[i].user +
                        // '" data-idcard="' + data[i].email + '" data-idcard="' + data[i].role +
                        // '" data-idcard="' + data[i].bu + '" data-idcard="' + data[i].cft +
                        // '" data-idcard="' + data[i].factory + '" onclick="showModalUpdateUser(this)"></i>' +
                        '<i class="fa fa-trash text-danger btnDeleteUser" onclick="showModalDeleteUser(\'' +
                        data[i].user + '\',' + data[i].id + ')"></i>' +
                        '</td>' +
                        '</tr>';
                }
                $('#tblListUsers tbody').html(html);
                if (widthScreen >= 1920) {
                    tbListUsers = $('#tblListUsers').DataTable({
                        "paging": true,
                        "pageLength": 20,
                        "lengthChange": false,
                        "searching": true,
                ...
                $('#inputSearch').on('keyup', function () {
                    tbListUsers.search($(this).val()).draw();
                });
            },
            error: function (err) {
                console.log(err)
    ...
    function showModalDeleteUser(idCard, idUser) {
        dataset.idUser = idUser * 1; //moi khi nguoi dung click vao nut xoa se dong thoi gan gia tri idUser
        $('.title-idcard-delete').html(idCard);
    ...
    function checkSMMSRole() {
        if (dataset.smmsRole == 'MPM') {
            $('.btnUploadFileUser').remove();
        } else if (dataset.smmsRole == 'BUYER') {
            $('.btnUploadFileUser').remove();
        }
    }

    $('.btnDownloadFileUser').on('click', function () {
        $('#tblListUsers_wrapper .dt-buttons > .dt-button').trigger('click');
    });
