...
function loadProgressStatus(factory, customerName, modelName, timeSpan) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            dataset.timeout['loadProgressStatus'] = setTimeout(loadProgressStatus, 180000, factory, customerName, modelName, timeSpan);
        }
    });
}

function loadStatus(factory, customerName, timeSpan) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            dataset.timeout['loadStatus'] = setTimeout(loadStatus, 180000, factory, customerName, timeSpan);
        }
    });
}
    ...
    if (!$.isEmptyObject(dataLocked)) {
        ...
        for (i = 0; i < dataLocked.length; i++) {
            ...
            stationLContent += '<div class="col-xs-6 col-sm-3 col-lg-2 card cldelsttlock countStation' + countHighchartsID + '" model="' + dataLocked[i].modelName + '" ' + type + '><input class="modelNameHide hide" type="radio" value="' + dataLocked[i].modelName + '">'
                ...
                + (type == 'locked' ? ('<span class="lockType" title="Error times in row">' + (dataLocked[i].status.substring(dataLocked[i].status.lastIndexOf(' ') + 1)) + '</span>') : '')
...
function showByModel() {
    ...
    if (dataset.selectedModel != selectedModel) {
        ...
        if (dataset.selectedModel == "all") {
            loadProgressStatus(dataset.factory, dataset.customerName, '', dataset.timeSpan);
        } else {
            loadProgressStatus(dataset.factory, dataset.customerName, dataset.selectedModel, dataset.timeSpan);
        }
        show();
    }
            ...
            if (!$.isEmptyObject(data)) {
                ...
                for (i in data) {
                    ...
                    htmlTbl += '<tr class="row-model" data-model="' + data[i].modelName + '">' +
                        ...
                        '<td class=" styletextshad ' + convertStatus(data[i].ete, 95, 98) + '" title="Click to move page E.T.E Detail"><a class="class-model-name ete" data-model="' + data[i].modelName + '" data-type="ete" >' + ete.toFixed(2) + '%</a></td>' +
                        ...
                        '<td class=" styletextshad ' + convertStatus(fpr, 90, 97) + '" title="Click to move page Station Detail"><a class="class-model-name fpr" data-model="' + data[i].modelName + '" data-type="rtr" >' + fpr.toFixed(2) + '%</a></td>' +
                        '<td class=" styletextshad ' + convertStatus((100 - rtr), 100 - data[i].retestRateTarget, 100 - data[i].retestRateTarget / 2) + '" title="Click to move page Retest Rate Detail"><a class="class-model-name rtr" data-model="' + data[i].modelName + '" data-type="rtr" >' + rtr.toFixed(2) + '%</a></td>' +
                        '<td>' + ffail + '</td>' +
                        '<td><a class="class-model-name" data-toggle="modal" data-target="#modalErrorSN" data-model="' + data[i].modelName + '" data-type="fail" >' + sfail + '</a></td>'+
                        '</tr>';
                ...
                $('.model-name').on('click', function (event) {
                    var url = "/station-detail?factory=" + dataset.factory + "&customerName=" + dataset.customerName + "&modelName=" + this.dataset.model + "&timeSpan="+dataset.timeSpan;
                    openWindow(url);
                });

                $('.class-model-name').on('click', function (event) {
                    if (this.dataset.type == 'fail') {
                        $("#modalErrorSN").show();
                        loadDataTableErrorFailFromJson({factory: dataset.factory, modelName: this.dataset.model, timeSpan: dataset.timeSpan})
                    } else {
                        var url = '';
                        if(this.dataset.type == 'fpr')
                            ...
                            url = "/ete-detail?factory=" + dataset.factory + "&customerName=" + dataset.customerName + "&modelName=" + this.dataset.model+ "&timeSpan=" + dataset.timeSpan;
                        else url = "/rtr-detail?factory=" + dataset.factory + "&customerName=" + dataset.customerName + "&modelName=" + this.dataset.model+ "&timeSpan=" + dataset.timeSpan;
                        // openWindow(url);
...
function loadModelListBefor() {
    $('.loader').css('display', 'block');
    $.ajax({
        type: "GET",
        url: "/api/test/model/daily/hour",
        data: {
            factory: dataset.factory,
            customerName: dataset.customerName,
            timeSpan: dataset.timeSpan,
        ...
        success: function (data) {
            if (!$.isEmptyObject(data)) {
                for(i in data) {
                    var ete = parseFloat($('.class-model-name.ete[data-model="' + data[i].modelName + '"]').html());
                    var eteB = parseFloat(data[i].ete.toFixed(2));
                    if(eteB > ete) {
                        $('.class-model-name.ete[data-model="' + data[i].modelName + '"]').parent().append('<span class="pull-right"><i class="fa fa-arrow-up"></i></span>');
                    } else if(eteB == ete) {
                        $('.class-model-name.ete[data-model="' + data[i].modelName + '"]').parent().append('<span class="pull-right"><i class="fa fa-minus"></i></span>');
                    } else {
                        $('.class-model-name.ete[data-model="' + data[i].modelName + '"]').parent().append('<span class="pull-right"><i class="fa fa-arrow-down"></i></span>');
                    }
                    var fpr = parseFloat($('.class-model-name.fpr[data-model="' + data[i].modelName + '"]').html());
                    var fprB = parseFloat(data[i].firstPassRate.toFixed(2));
                    if(fprB > fpr) {
                        $('td>.class-model-name.fpr[data-model="' + data[i].modelName + '"]').append('<span class="pull-right"><i class="fa fa-arrow-up"></i></span>');
                    } else if(fprB == fpr) {
                        $('td>.class-model-name.fpr[data-model="' + data[i].modelName + '"]').append('<span class="pull-right"><i class="fa fa-minus"></i></span>');
                    } else {
                        $('td>.class-model-name.fpr[data-model="' + data[i].modelName + '"]').append('<span class="pull-right"><i class="fa fa-arrow-down"></i></span>');
                    }
                    var rtr = parseFloat($('.class-model-name.rtr[data-model="' + data[i].modelName + '"]').html());
                    var rtrB = parseFloat(data[i].retestRate.toFixed(2));
                    if(rtrB > rtr) {
                        $('td>.class-model-name.rtr[data-model="' + data[i].modelName + '"]').append('<span class="pull-right"><i class="fa fa-arrow-up"></i></span>');
                    } else if(rtrB == rtr) {
                        $('td>.class-model-name.rtr[data-model="' + data[i].modelName + '"]').append('<span class="pull-right"><i class="fa fa-minus"></i></span>');
                    } else {
                        $('td>.class-model-name.rtr[data-model="' + data[i].modelName + '"]').append('<span class="pull-right"><i class="fa fa-arrow-down"></i></span>');
                    }
                }
            }
        },
...
function loadDataTableErrorFailFromJson(requestData) {
    $.ajax({
        type: "GET",
        url: "/api/test/station/error/fail",
        data: requestData,
        contentType: "application/json; charset=utf-8",
        success: function (json) {
            $('#tbl-error-fail>tbody>tr').remove();

            var body = $('#tbl-error-fail>tbody');
            if (!$.isEmptyObject(json)) {
                for (i in json) {
                    body.append('<tr><td>' + (1 + Number(i)) + '</td><td>' + json[i].time + '</td><td>' + json[i].groupName + '</td><td>' + json[i].stationName + '</td><td>' + json[i].tester + '</td><td>' + toString(json[i].chamber) + '</td><td><a data-serial="' + json[i].serial + '" data-toggle="modal" data-target="#modal_serial_tracking" onclick="loadSerialTracking(this.dataset.serial)">' + json[i].serial + '</a></td><td>' + json[i].errorCode + '</td><td>' + toString(json[i].reasonCode) + '</td></tr>');
                }
            } else {
