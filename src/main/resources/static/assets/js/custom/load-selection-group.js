...
//         contentType: "application/json; charset=utf-8",
//         success: function(data){
//             var list = '<li role="presentation" ><a role="menuitem" tabindex="-1" href="#" name="ALL">Select All</a></li>';
//             for(i in data){
//                 list += '<li role="presentation" ><a role="menuitem" tabindex="-1" href="#" name="'+i+'">'+ data[i] +'</a></li>';
    ...
    $.ajax({
        ...
        success: function(data){
            if(!$.isEmptyObject(data)){
                ...
                selector.append(options);
                selector.val(dataset.modelList);
                selector.selectpicker('refresh');
            }
        },
        failure: function(errMsg) {
...
function loadSelectLine(){
    var url = '/api/test/sfc/line?factory=' + dataset.factory + '&customer=' + dataset.customer + '&stage=' + dataset.stage + '&shiftType=' + dataset.shiftType + '&workDate=' + dataset.workDate;
    for(i in dataset.modelList) {
        url += '&modelList=' + dataset.modelList[i];
    }
    $.ajax({
        type: "GET",
        url: url,
        ...
        success: function(data){
            ...
            var selector = $("select[name='lineName']");
            selector.children('option').remove();
            var options = "";
            for(i in data){
                options+='<option value=' + data[i] + '>' + data[i] + '</option>';
...
function loadSelectMO(){
    var url = '/api/test/sfc/mo?factory=' + dataset.factory + '&customer=' + dataset.customer + '&stage=' + dataset.stage + '&shiftType=' + dataset.shiftType + '&workDate=' + dataset.workDate;
    for(i in dataset.modelList) {
        url += '&modelList=' + dataset.modelList[i];
    }
    $.ajax({
        ...
        success: function(data){
            ...
            var selector = $("select[name='moName']");
            selector.children('option').remove();
            var options = "";
            for(i in data){
                options+='<option value=' + data[i] + '>' + data[i] + '</option>';
