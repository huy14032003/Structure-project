...
// });
var editRC, editA, editD, editOwner, errorC, data;
        ...
        for (i in keys) {    
            ...
            if(json[keys[i]].rootCause==null || json[keys[i]].action==null || json[keys[i]].description==null){
                body+=  '<td><textarea style="white-space:pre" id="txtEditRC'+stt+'" class="form-control hidden" data-idt="'+json[keys[i]].idNoteError+'" oninput="textwrite('+stt+')" name="" type="text" value="" ></textarea>'
                        ...
                        +   '<span id="errorDescription'+stt+'" value="'+ (json[keys[i]].factory == 'B06' ? "" : json[keys[i]].errorDescription) +'">'+ (json[keys[i]].factory == 'B06' ? "" : json[keys[i]].errorDescription) +'</span></td>'
                        +   '<td><textarea id="txtEditOwner'+stt+'" class="form-control hidden" name="" type="text" value="" oninput="textwrite('+stt+')"></textarea>'
                        +   '<span id="owner'+stt+'" value=""></span></td>'
            }else{
                ...
                tmpOwner = json[keys[i]].owner == null ? '' : json[keys[i]].owner.replace(/<br\s*\/?>/g,'\n');
                ...
                body+=  '<td><textarea id="txtEditRC'+stt+'" class="form-control hidden" oninput="textwrite('+stt+')" data-idt="'+json[keys[i]].idNoteError+'" name=""  type="text" value="text break text">'+ tmpRC +'</textarea>'
                        ...
                        +   '<span id="errorDescription'+stt+'" value="'+ (json[keys[i]].factory == 'B06' ? json[keys[i]].description : json[keys[i]].errorDescription) +'">'+ (json[keys[i]].factory == 'B06' ? json[keys[i]].description : json[keys[i]].errorDescription) +'</span>'
                        +   '</td>'
                        +   '<td><textarea id="txtEditOwner'+stt+'" class="form-control hidden" name="" type="text" value="" oninput="textwrite('+stt+')">'+tmpOwner+'</textarea>'
                        +   '<span id="owner'+stt+'" value="'+ (json[keys[i]].owner == null ? '' : json[keys[i]].owner) +'">'+ (json[keys[i]].owner == null ? '' : json[keys[i]].owner) +'</span></td>'
            }
...
function edit(stt) {
    ...
    $("#owner" + stt).addClass("hidden");
    $("#txtEditOwner" + stt).removeClass("hidden");
...
function textwrite(stt){
    ...
    editOwner = $('#txtEditOwner'+stt).val().replace(/\r\n|\r|\n/g,"<br />");
    console.log(editRC);
}
        ...
        success: function (data) {
            ...
            for (i in data) {
                ...
                dataChart[i] = {
                    ...
                    totalTestMachine: data[i].stationList.length
                }
            ...
            Highcharts.chart('chart-group-overview', {
                ...
                tooltip: {
                    ...
                    pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y:.2f}%</b> <br/> Input: <b>{point.wip}</b> <br/> Output: <b>{point.pass}</b> <br/> First Fail: <b>{point.firstFail}</b> <br/> Second Fail: <b>{point.secondFail}</b> <br/> Yield Rate: <b>{point.yieldRate:.2f}%</b> <br/> First Pass Rate: <b>{point.firstPassRate:.2f}%</b> <br/> Total Test Machine: <b>{point.totalTestMachine}</b>'
                },
                legend: {
                    enabled: false
                },
        ...
        success: function (data) {
            ...
            for (j in keys) {
                ...
                if (!$.isEmptyObject(value)) {
                    ...
                    yieldRate[j] = { name: keys[j], y: ele, groupList: value.groupList};
                } else {
                    yieldRate[j] = { name: keys[j], y: 0, groupList: []};
                }
            }
            ...
            Highcharts.chart('chart-group-name', {
                ...
                tooltip: {
                    ...
                    //pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y:.2f}%</b>' + pointFormat,
                    formatter: function() {
                        var pointFormat = '<span style="font-size:11px; color:'+ this.point.color +'">'+ dataset.modelName +'-'+ this.point.name +':'+'<b>'+ this.point.y.toFixed(1) +'%</b>'+'</span>';
                        for (k in this.point.groupList) {
                            pointFormat += '<br/><span style="color:{point.color}">'+ this.point.groupList[k].groupName +'</span>: <b>'+ this.point.groupList[k].retestRate.toFixed(1) +'%</b>'
                        }
                        return pointFormat;
                    }
                },
...
function loadHighChartErrorAll(data) {
    var keys = Object.keys(data);
    var errorList = new Array(keys.length);
    for (j in keys) {
        var value = data[keys[j]];
        if (!$.isEmptyObject(value)) {
            errorList[j] = { name: keys[j], y: value.testFail, fail: value.fail};
        } else {
            errorList[j] = { name: keys[j], y: 0, fail: 0};
        }
    }

    Highcharts.chart('chart-error-all', {
        chart: {
            type: 'column',
            style: {
                fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif,"Apple Color Emoji","Segoe UI Emoji","Segoe UI Symbol","Noto Color Emoji"',
        ...
        title: {
            text: dataset.groupName + ' DEFECTED',
            style: {
                fontSize: '16px',
                fontWeight: 'bold'
            }
...
//    });
    ...
    loadChartErrorHourly(requestData, top1ErrorByHourChart, top1ErrorByHourProps);
    loadChartErrorWeekly(requestData, top1ErrorByWeekChart, top1ErrorByWeekProps);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (!$.isEmptyObject(data)) {
                ...
                loadDataTableErrorFailFromJson({factory: dataset.factory, modelName: dataset.modelName, groupName: dataset.groupName, stationName: dataset.stationName, timeSpan: dataset.timeSpan});
            } else {
                dataset.errorCode = '';
                dataset.errorDesc = '';
...
function loadChartErrorHourly(requestData, top1ErrorByHourChart, top1ErrorByHourProps) {
    $.ajax({
        type: "GET",
        url: "/api/test/station/hourly/error/top1",
        data: {
...
function loadChartErrorWeekly(requestData, top1ErrorByWeekChart, top1ErrorByWeekProps) {
    $.ajax({
        type: "GET",
        url: "/api/test/station/weekly/error/top1",
        data: {
