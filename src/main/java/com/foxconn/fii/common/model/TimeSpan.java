...
public class TimeSpan {
    ...
    public long getTimeRelax(Date starTime ,Date endTime) throws ParseException {
        ...
        String sdate = fm.format(starTime);
        String edate = fm.format(endTime);
        long time=0;
        Date now=new Date();
        String nowstr = format.format(now);
        if((format.parse(nowstr)).after(format.parse(edate+" "+"06:30"))) {
            time += 300;
        }
        else if((format.parse(nowstr)).after(format.parse(edate+" "+"04:30")))
        {
            time += 240;
        }
        else if((format.parse(nowstr)).after(format.parse(edate+" "+"02:30")))
        {
            time += 230;
        }
        else if((format.parse(nowstr)).after(format.parse(edate+" "+"00:30")))
        {
            time += 220;
        }else if((format.parse(nowstr)).after(format.parse(sdate+" "+"21:30")))
        {
            time += 160;
        }else if((format.parse(nowstr)).after(format.parse(sdate+" "+"19:30")))
        {
            time += 150;
        }else if((format.parse(nowstr)).after(format.parse(sdate+" "+"16:30")))
        {
            time += 90;
        }else if((format.parse(nowstr)).after(format.parse(sdate+" "+"14:30")))
        {
            time += 80;
        }else if((format.parse(nowstr)).after(format.parse(sdate+" "+"12:30")))
        {
            time += 70;
        }else if((format.parse(nowstr)).after(format.parse(sdate+" "+"09:30")))
        {
            time += 10;
        }
        return time;
